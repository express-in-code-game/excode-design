(ns starnet.alpha.worker
  (:require
   ...
   #_[starnet.alpha.common.game.data :refer [make-entities]]))
  ...
  (go (loop []
        (if-let [v (<! ch-ops-in)]
          (let [{:keys [worker/op ch/c-out worker/args]} v]
            (condp = op
              :starnet.alpha.common.game.data/make-entities (let [o nil #_(make-entities args)]
                                                              (>! c-out o)))))
        (recur))))
